...
# design-grid resolution that enforces the number of grid points to be the same as those in RETICOLO/RCWA design
design_region_resolution = (117/px, 44/py)
...
input_flux = mp.get_fluxes(flux) # incident flux
sim.reset_meep()
...
def mapping(x):
    filtered_field = mpa.conic_filter(x,filter_radius,px,py,design_region_resolution, periodic_axes=(0,1))
...
Nx, Ny, Nz = int(round(design_region_resolution[0]*px))+1, int(round(design_region_resolution[1]*py))+1, 1
    
design_variables = mp.MaterialGrid(mp.Vector3(Nx, Ny, Nz), mp.air, Si, grid_type="U_MEAN")
...
geometry = [mp.Block(center=mp.Vector3(0,0,-0.5*sz+0.5*(dpml+dsub)),
                     size=mp.Vector3(px,py,dpml+dsub),
                     material=SiO2),
...
opt = mpa.OptimizationProblem(simulation=sim,objective_functions=J,objective_arguments=ob_list,
                              design_regions=[design_region],fcen=fcen,df=0,nf=1,decay_by=1e-8)
...
def f(v, gradient):
    ...
    np.savetxt('objective_2d.dat',evaluation_history) # evaluation history of the objective, i.e., the diffraction efficiency
    np.savetxt('pattern_2d.dat',np.vstack((v,v_mapped))) # unfiltered and filtered design patterns at the current iteration
...
# If the optimization needs to be started from a given struture, e.g., one of the structures optimized by RETICOLO/RCWA,
# the initial structure can be set as follows.
#input_structure = np.loadtxt('designs/device1.csv', delimiter=',').flatten()
#x = mpa.unfilter_design(input_structure, processing = lambda x: mpa.tanh_projection(mapping(x), betas[0], eta_i), maxiter = 500)
