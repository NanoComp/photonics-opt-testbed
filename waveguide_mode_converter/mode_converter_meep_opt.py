...
def c(result, x, gradient, eta, beta, use_epsavg):
    ...
    f0_merged_str = '[' + ','.join(str(fm) for fm in f0_merged) + ']'
    print(
...
if __name__ == "__main__":
    ...
    x = np.insert(x, 0, 1.2)  # ignored; actual value determined by objective and constraint functions below
    ...
    beta_thresh = 65 # threshold beta above which to use subpixel smoothing
    ...
    for beta, max_eval in zip(betas, max_evals):
        ...
        x[0] = np.amax(t0)
        x[0] = 2.0 * (max(x[0], t) if beta == betas[-1] else x[0])
