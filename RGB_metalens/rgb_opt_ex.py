...
savepath = 'Ex/media'+str(medianum)
mp.verbosity(0)
...
def mapping(x,eta,beta):
    xc = npa.where(bottom_mask.flatten(),0,npa.where(Air_mask.flatten(),0,x))
...
geometry = [mp.Block(center=design_region.center, size=design_region.size, material=design_variables),
                mp.Block(center=mp.Vector3(0, -2.5), size=mp.Vector3(Sx, 2), material=Si)]
